...
def add_new_tache():
    ...
    # Côté interface :
        # Label nom :
label_new_tache_nom = ttk.Label(fenetre, text='Nom de la nouvelle tâche :')
label_new_tache_nom.grid(column=0, row=0)
        # Champ nom :
new_tache_nom = ttk.Entry(fenetre)
new_tache_nom.grid(column=0, row=1)

        # Label deadline :
label_new_tache_deadline = ttk.Label(fenetre, text='Date de fin prévue (YYYY-MM-DD):')
label_new_tache_deadline.grid(column=0, row=2)
        # Champ deadline :
new_tache_deadline = ttk.Entry(fenetre)
new_tache_deadline.grid(column=0, row=3)

        # Label état :
label_new_tache_etat = ttk.Label(fenetre, text='État :')
...
listeAffich = ttk.Combobox(fenetre, values=liste_etat)
